...
class IPAddress {
  ...
  static fromNumber(number, type) {
    ...
    if (type == 4) {
      let octets = [];
      for (let i = 0; i < CONSTANTS.IPV4_OCTET_GROUPS; i++) {
        octets.push(
          (number >> BigInt(8 * (CONSTANTS.IPV4_OCTET_GROUPS - 1 - i))) %
    ...
    if (type == 6) {
      let octets = [];
      for (let i = 0; i < CONSTANTS.IPV6_OCTET_GROUPS; i++) {
        octets.push(
          (number >> BigInt(16 * (CONSTANTS.IPV6_OCTET_GROUPS - 1 - i))) %
  ...
  /**
   * Increment the currently stored IP address by the given number and return a new IPAddress instance.
   * Won't modify the current instance.
   * @param {number} by The number to increment the IP address by.
  ...
  /**
   * Decrement the currently stored IP address by the given number and return a new IPAddress instance.
   * Won't modify the current instance.
   * @param {number} by The number to decrement the IP address by.
